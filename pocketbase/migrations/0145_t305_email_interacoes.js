...
// na oportunidade. Coleção `interacoes_email` append-only (deleteRule null),
// mesmo padrão de interacoes_whatsapp (T3.03).
// Campos: negocio (relation obrigatória), contato (relation opcional),
// direcao (entrada|saida), assunto (3–300 chars), resumo (5–5000 chars),
// resultado (sem_resposta|resposta|reuniao_agendada|proposta_solicitada|negativo),
...
migrate(
  (app) => {
    ...
    var existe = true
    try {
      app.findCollectionByNameOrId('interacoes_email')
    } catch (_) {
      existe = false
    }
    if (!existe) {
      var col = new Collection({
        name: 'interacoes_email',
        type: 'base',
        ...
        fields: [
          ...
          { name: 'assunto', type: 'text', required: true, max: 300 },
        ...
        indexes: [
          'CREATE INDEX idx_email_negocio ON interacoes_email (negocio)',
          'CREATE INDEX idx_email_created ON interacoes_email (created)',
        ],
      })
      app.save(col)
    }
  ...
  (app) => {
    try {
      app.delete(app.findCollectionByNameOrId('interacoes_email'))
    } catch (_) {}
  },
)
