...
// Runtime goja: lógica inline no callback; datas PB " " → "T"; 0001-01-01 = ausente.
...
// ---- Lógica central (inline no escopo do arquivo — chamada pelo cron e pela rota) ----
function executarAutomacoes() {
  ...
  try {
    ...
    return { erro: 'Coleção de execuções indisponível: ' + String(err) }
  }
  ...
  // mapa: negocio -> permanência aberta (entrou_em)
  ...
  try {
    ...
    return { erro: 'Falha ao consultar oportunidades: ' + String(err) }
  }

  // propostas emitidas por negócio (mais recente)
  var propostas = []
  try {
  ...
  var inserir = function (regra, negocio, responsavel, detalhe) {
    // idempotência: já existe regra+negócio+dia?
    try {
  ...
  for (var i = 0; i < negocios.length; i++) {
    ...
    // --- follow_up_proposta / follow_up_sem_resposta ---
