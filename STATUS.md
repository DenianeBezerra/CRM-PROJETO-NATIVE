...
**Task ativa:** T3.17 — Modelo de contrato no CRM (SPEC-3-017) — implementada, em correção (divergência do pod no endpoint de geração)
...
**Versão atual:** v0.0.550 (QA verde; geração de contrato pendente de debug no pod)
...
| 17   | T3.17 — Modelo de contrato no CRM (passo anterior ao ClickSign)               | SPEC-3-017            | 🔧 Em correção — pod divergente |
